...
const AdminPortalForm = ({ data }) => {
  ...
  const addData = async (data) => {
    try {
      const response = await createStudent(data).then(() =>
  ...
  const handleSubmit = async (e) => {
    ...
    // this checks  the data inputed in the student
    if (isUpdate) {
      updateData(formData);
      adminContextDispatch({ type: OPEN_UPDATE_FORM });
    } else {
      addData(formData);
      adminContextDispatch({ type: OPEN_UPDATE_FORM });
      e.target.reset();
